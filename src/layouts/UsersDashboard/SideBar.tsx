...
export const SideBar = ():JSX.Element => {
    ...
    return (
        <aside className="w-[16rem] flex flex-col bg-[#0E2A22] p-4 overflow-y-auto text-white shadow-2xl h-screen">
            ...
            <ul className="space-y-6">
                {
                    menu.filter((item) => !item.superAdminOnly || superAmin === "SUPER-ADMIN").map((item) => (
                        ...
                        className={`p-3 rounded-lg cursor-pointer transition ${active === item.title ? "bg-blue-600" : "hover:bg-blue-600/20"}`}
                        >
                            {
                                item.title === "Logout" ? (
